...
/**
 * This functions get all assertions that does not have any outcome and
 * set a fixed outcome for it
 */
EarlDocument.prototype.noResultToOutcome = function(outcome) {
    for(var i=0; i < this.assertions.length; i++) {
        if (!this.assertions[i].hasResult()) {
            this.assertions[i].addResult(outcome);
        }
    }
} 
 ...
 * 
 */
EarlDocument.prototype.noResultToPassed = function() {
    this.noResultToOutcome(EarlOutcomeValue.PASSED);
} 
...
/**
 * Returns true or false if a result has been added to this assertion
 */
EarlAssertion.prototype.hasResult = function () {
    return (this.result.identifier != undefined)
}
